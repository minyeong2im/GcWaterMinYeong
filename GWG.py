...
def 총요금계산(업종: str, 사용량: int, 구경: int, 세대수:int=1, 하수도여부: bool=True, 겸업여부: bool=False, 주기: str = "격월", 감면세대수:int=0, 누수감면여부: bool=False, 직전사용량_1: int=0, 직전사용량_2: int=0) -> dict:
    ...
    상수도복지 = 복지감면["상수도감면"] * -1
    하수도복지 = 복지감면["하수도감면"] * -1 if 하수도여부 else 0
    부담금복지 = 복지감면["부담금감면"] * -1
    ...
    if 겸업여부:
        ...
        # 자동이체 할인
        자동이체할인 = max(5000, (겸업_상수도합 + 정액료) * 0.01) * -1
        # 상수도 자동이체 할인 적용된 금액
        자_상수도합 = 겸업_상수도합 + 자동이체할인
        자_총요금계산 = 정액료 + 자_상수도합 + 겸업_하수도합 + 물이용부담금
        ...
        if 누수감면여부:
            ...
            하수도누수감면 = (rounddown_10(max(0, 겸업_하수도합 - 하수도평균금액)) * -1) if 하수도여부 else 0
            ...
            감면적용하수도 = max(0, 누수감면된하수도 + 하수도복지) if 하수도여부 else 0
            감면적용부담금 = max(0, 누수감면된부담금 + 부담금복지)
            합산_금액 = 정액료 + 감면적용상수도 + 감면적용하수도 + 감면적용부담금
            # 감면자동이체할인
            감면자동이체할인 = max(5000, (누수감면된상수도 + 정액료) * 0.01) * -1
            # 누수감면적용된 자동이체할인상수도금액
            자_감면상수도합 = 누수감면된상수도 + 감면자동이체할인
            # 감면자동이체할인적용된상수도합
            자_감면적용상수도 = max(0, 누수감면된상수도 + 상수도복지 + 감면자동이체할인)
            # 과오납금액
            자동이체_상수도과오납 = 자_상수도합 - 자_감면상수도합
            상수도과오납 = 겸업_상수도합 - 누수감면된상수도
            하수도과오납 = 겸업_하수도합 - 누수감면된하수도 if 하수도여부 else 0
            물이용부담금과오납 = 물이용부담금 - 누수감면된부담금
            ...
            감면적용하수도 = max(0, 겸업_하수도합 + 하수도복지) if 하수도여부 else 0
        ...
        기본총합계 = 정액료 + 이용요금합계
        # 자동이체 할인
        자동이체할인 = max(5000, (상수도이용요금 + 정액료) * 0.01) * -1
        # 상수도 자동이체 할인 적용된 금액
        자_상수도합 = 상수도이용요금 + 자동이체할인
        자_총요금계산 = 정액료 + 자_상수도합 + 하수도이용요금 + 물이용부담금
        if 누수감면여부:
            직전평균 = 직전평균요금계산(업종, 세대수, 주기, 하수도여부, 겸업여부, 직전사용량_1, 직전사용량_2)
            ...
            하수도누수감면 = (rounddown_10(max(0, 하수도이용요금 - 하수도평균금액)) * -1) if 하수도여부 else 0
            ...
            # 과오납금액
            자동이체_상수도과오납 = 자_상수도합 - 자_감면상수도합
            상수도과오납 = 상수도이용요금 - 누수감면된상수도
            하수도과오납 = 하수도이용요금 - 누수감면된하수도 if 하수도여부 else 0
            물이용부담금과오납 = 물이용부담금 - 누수감면된부담금
            return {
                "정액료": 정액료,
                "상수도이용요금": 상수도이용요금,
...
st.set_page_config(page_title="김천시 수도요금 계산기", layout="wide")
st.title("💧 김천시 수도요금 계산기")

# 입력 영역
st.subheader("📥 요금 입력 항목")

col1, col2 = st.columns(2)

with col1:
    업종 = st.selectbox("업종 선택", ["가정용", "일반용", "대중탕용", "전용공업용", "산업용"])
    사용량 = st.number_input("사용량(㎥)", min_value=0, step=1)
    구경 = st.selectbox("계량기 구경(㎜)", [13, 20, 25, 40, 50, 75, 100, 150, 200, 250, 300])
    세대수 = st.number_input("세대수", min_value=1, step=1, value=1)
    주기 = st.radio("검침 주기", ["매월", "격월"])
    
with col2:
    하수도여부 = st.checkbox("하수도 부과 여부", value=True)
    겸업여부 = st.checkbox("겸업용 수용가", value=False)
    감면세대수 = st.number_input("복지 감면 세대수", min_value=0, step=1)
    누수감면여부 = st.checkbox("누수 감면 적용 여부", value=False)

    st.markdown("---")
    st.markdown("**직전 사용량 입력 (누수감면용)**")
    직전1 = st.number_input("직전 사용량 1회차", min_value=0, step=1)
    직전2 = st.number_input("직전 사용량 2회차", min_value=0, step=1)

# 버튼
if st.button("요금 계산하기"):
    with st.spinner("요금을 계산 중입니다..."):
        결과 = 총요금계산(
            업종=업종,
            사용량=사용량,
            구경=구경,
    ...
    st.write(f"💵 **총 합산 금액:** {결과.get('총합산금액', 결과.get('합산_금액', 0)):,} 원")
    st.write(f"📦 **정액료:** {결과['정액료']:,} 원")
    st.write(f"🚰 **상수도 요금:** {결과.get('상수도합산금액', 0):,} 원")
    st.write(f"🚿 **하수도 요금:** {결과.get('하수도합산금액', 0):,} 원")
    st.write(f"🌊 **물이용부담금:** {결과.get('부담금합산금액', 결과.get('물이용부담금', 0)):,} 원")

    # 필요 시 세부 내역 추가 표시
    with st.expander("🔍 세부 내역 보기"):
        for key, value in 결과.items():
            st.write(f"{key} : {value:,} 원" if isinstance(value, (int, float)) else f"{key} : {value}")